...
dbController.verifyUser = (req, res, next) => {
    ...
    User.findOne({ username }, function (err, response) {
      if (err) {
        ...
        bcrypt.compare(password, data.password, function (err, compareResult) {
          if (err) {
...
dbController.getFavorites = (req, res, next) => {
  ...
  console.log('within dbController.getFavorites');
  User.findOne({ username }, function (err, response) {
    if (err) {
      console.log(`Error in dbController.getFavorites: ${err}`);
      return next(err);
    } else if (response === null) {
      console.log(`getFavorites returned no user`);
      return next();
    } else {
      res.locals.favorites = resposne.favorites;
    }
  });
}

dbController.addFavorite = (req, res, next) => {
  const { favorites } = req.body;
  console.log('within dbController.getFavorites');
  User.findOneAndUpdate({ username }, { favorites }, { new: true }, function (err, response) {
    if (err) {
      console.log(`Error in dbController.addFavorite: ${err}`);
      return next(err);
    } else {
      console.log('addFavorite returned no user');
      return next();
    }
  });
}
